...
import { addCompetition } from '../../services/api'; // Pastikan ini sesuai dengan API yang digunakan

const AddPerlombaanModal = ({ show, onHide, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      kategori: formData.kategori.split(',').map(item => item.trim()),
      syarat_ketentuan: formData.syarat_ketentuan.split(',').map(item => item.trim()),
      cara_mendaftar: formData.cara_mendaftar.split(',').map(item => item.trim()),
      timeline: {
        pendaftaran_mulai: new Date(formData.timeline.pendaftaran_mulai).toISOString(),
        pendaftaran_selesai: new Date(formData.timeline.pendaftaran_selesai).toISOString(),
        pengumpulan_karya: new Date(formData.timeline.pengumpulan_karya).toISOString(),
        deadline_karya: new Date(formData.timeline.deadline_karya).toISOString(),
        penjurian: formData.timeline.penjurian,
        pengumuman: new Date(formData.timeline.pengumuman).toISOString(),
      },
    };
    
    try {
      await addCompetition(payload);
      alert('Perlombaan berhasil ditambahkan!');
      onSuccess();
      onHide();
    } catch (error) {
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg" backdrop="static">
      <Modal.Header closeButton>
        <Modal.Title>Tambah Perlombaan Baru</Modal.Title>
      </Modal.Header>
...
export default AddPerlombaanModal;